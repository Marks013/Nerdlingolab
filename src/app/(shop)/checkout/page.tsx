...
import { getCustomerCheckoutProfile, getCustomerSavedAddresses } from "@/lib/orders/queries";
...
export default async function CheckoutPage(): Promise<React.ReactElement> {
  ...
  const customerProfile = await getCustomerCheckoutProfile(session.user.id);
  const savedAddresses = (await getCustomerSavedAddresses(session.user.id)).map((address) => ({
    id: address.id,
    label: address.label,
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <CheckoutClient customerProfile={customerProfile} savedAddresses={savedAddresses} />
    </main>
  );
}
